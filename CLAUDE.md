...
| **Slicing** | `slice_all`, `get_slicing_status`, `export_gcode`, `get_print_estimate` (`printed_layers` = the G-code's layer count; `object_layers` / `support_layers` split; `layer_count` deprecated, now the printed count too) |
...
| `src/slic3r/GUI/OrcaMCP/OrcaMCPRenderMath.cpp` | Pure render math: pixel projection, palette, camera presets and fit (`frame_camera`), plate-view filter and blank-image hint, grid, temp image paths (unit-tested in `tests/slic3rutils/test_render_math.cpp`) |
