...
| **Config** | `get_presets`, `get_edited_presets`, `select_preset` (`type: printer` returns the resulting `filaments` and `colors_source`: `remembered`/`default`/`kept`), `apply_config`, `clone_preset`, `save_preset`, `delete_preset`, `reset_preset`, `get_valid_config_keys` |
