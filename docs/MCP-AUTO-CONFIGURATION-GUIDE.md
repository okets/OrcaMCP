...
## Implementation Notes: OrcaMCP (C++)

OrcaMCP implements MCP auto-configuration directly in C++ within the OrcaSlicer GUI. This differs from the TypeScript approach above but follows the same principles.

### Key Differences from TypeScript Implementation

| Aspect | TypeScript (npm) | OrcaMCP (C++) |
|--------|------------------|---------------|
| Server location | Global npm install | Bridge script in app bundle |
| Command | `your-server` | `python3 ~/.orcamcp/orcamcp-bridge.py` |
| Config UI | CLI commands | wxWidgets Preferences tab |
| Trigger | Terminal commands | "Connect AI" home screen button |

### Shared Path Architecture

OrcaMCP uses a **shared path approach** to handle dev/production switching:

```
App Bundle (dev or prod)                    Shared Location
┌─────────────────────────────┐            ┌─────────────────────────┐
│ OrcaSlicer.app/             │            │ ~/.orcamcp/             │
│   Contents/Resources/       │  copy on   │   orcamcp-bridge.py     │
│     scripts/                │ ─────────► │                         │
│       orcamcp-bridge.py     │  connect   │                         │
└─────────────────────────────┘            └─────────────────────────┘
                                                      │
                                                      │ configured in
                                                      ▼
                                           MCP client configs point here
...
├── MCPClientConfig.hpp    # Client metadata, enums, function declarations
├── MCPClientConfig.cpp    # ~620 lines: connect/disconnect, JSON/TOML handling
```
...
// Resolve platform-specific config paths
static std::string resolve_config_path(const std::string& template_path);

// Connect/disconnect client
static bool connect_client(MCPClientId id, std::string& error);
static bool disconnect_client(MCPClientId id, std::string& error);
```

### Platform-Aware Tab Indexing

The Preferences dialog has platform-conditional tabs. The "Associate" tab only exists on Windows:

```cpp
// GUI_App.cpp - homepage_connectai handler
#ifdef _WIN32
...
### Bridge Script Path Format

The bridge script is stateless and connects to localhost:13618:
...
```json
{
  "mcpServers": {
    "orca-slicer": {
      "command": "python3",
      "args": ["/Users/username/.orcamcp/orcamcp-bridge.py"]
    }
  }
}
```
...
- **MCPClientConfig**: `src/slic3r/GUI/OrcaMCP/MCPClientConfig.cpp`
- **Preferences tab**: `src/slic3r/GUI/Preferences.cpp` (search for `create_mcp_clients_page`)
- **Home screen handler**: `src/slic3r/GUI/GUI_App.cpp` (search for `homepage_connectai`)
- **Bridge script**: `resources/scripts/orcamcp-bridge.py`
...
- **December 2024**: OrcaMCP C++ implementation with shared path approach
