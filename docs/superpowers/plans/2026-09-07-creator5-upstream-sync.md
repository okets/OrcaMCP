...
- [ ] **Step 3: `README.md`** — rewrite these sections:
  ...
  - **Flashforge Creator 5 / 5 Pro**: printer setup (IP, serial number, check code from the printer's network screen), that the `Flashforge Creator 5 Pro` profile uses the local API on port 8898, the material-station mapping on send, Device tab support, and the natural-language example from Task 2.8. State plainly that Reforge/Klipper-modded printers should use the Moonraker host type instead.
