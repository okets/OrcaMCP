...
The bridge answers the very first tools/list from the checked-in scripts/orcamcp_tools.json when
OrcaSlicer is not up yet -- the normal case, because the MCP client starts first. A running app of
another build can list different tools. Without a tools/list_changed notification the client keeps
whatever the first list said for the life of the session, which is how get_presets' filters were
unreachable from a session whose running server had them (when that list was a hand-regenerated
tools_schema.py).
"""
