...
;    }
    this->Bind(wxEVT_CHAR_HOOK, [this](wxKeyEvent &evt) {
        ...
        if (evt.CmdDown() && evt.ShiftDown() && evt.GetKeyCode() == 'G') {
            ...
            if (m_print_enable) {
                // Plater::priv::on_action_print_plate picks the send dialog; see SendDialogRouting.hpp.
                // It falls through to send_gcode_legacy(), which is what EVT_GLTOOLBAR_SEND_GCODE did here.
...
void MainFrame::init_menubar_as_editor()
{
    ...
    wxMenu* fileMenu = new wxMenu;
    {
        ...
        // which nobody answers when an agent launched the app. Skip that read then: Home lists the
        // earlier projects without thumbnails, and projects saved or opened from now on get theirs.
        if (is_agent_launch())
            m_recent_projects.SkipExistingThumbnails();
        else
